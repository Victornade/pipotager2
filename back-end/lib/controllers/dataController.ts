...
export class DataController {
    ...
    getDatas(req, res) {
        console.log('get all');
        Data.find({date:{$gte:new Date("2019-02-17")}}, (err, data) => {
            if (err) {
                res.send(err);
                return;
            }
            ...
            var toSend = [
                { name: "temperature", fillColor: 'rgb(	255, 0, 0,0.5)', color: 'rgb(255, 0, 0,0.5)',   data: [] , yAxis:1, type: 'area', turboThreshold:0 },
                { name: "humidite_air", fillColor:'	rgb(65, 105, 225,0.5)', color: 'rgb(65, 105, 225,0.5)',   data: [] , yAxis:0, type: 'area', turboThreshold:0 },
